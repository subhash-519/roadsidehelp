...
public class BookingValidator {
    ...
    public void validateCreateBooking(Garage garage, Vehicle vehicle, String userId) {
        ...
        if (vehicle == null) {
            throw new ApiException(ErrorCode.VEHICLE_NOT_FOUND, "Vehicle not found");
        }

        if (!garage.isApproved()) {
            throw new ApiException(ErrorCode.GARAGE_NOT_VERIFIED, "Garage is not approved");
        }

        if (!garage.isOpen()) {
            throw new ApiException(ErrorCode.GARAGE_CLOSED, "Garage is currently closed");
        }

        if (!vehicle.getUser().getId().equals(userId)) {
            throw new ApiException(ErrorCode.VEHICLE_NOT_OWNED, "Vehicle does not belong to the user");
        }
    }
    ...
    public void validateMechanicAssignment(Booking booking) {
        ...
        if (booking.getStatus() != BookingStatus.ACCEPTED) {
            throw new ApiException(
                    ErrorCode.BOOKING_NOT_ACCEPT,
                    "Mechanic can only be assigned after booking is ACCEPTED"
            );
        }

        if (booking.getMechanic() != null) {
            throw new ApiException(
                    ErrorCode.MECHANIC_ALREADY_ASSIGNED,
                    "Mechanic already assigned to this booking"
            );
        }
    }
    ...
    public void validateUserCancellation(Booking booking) {
        ...
        if (booking.getStatus() == BookingStatus.IN_PROGRESS
            ...
            throw new ApiException(
                    ErrorCode.BOOKING_NOT_CANCELED,
                    "Booking cannot be canceled after service has started"
            );
        }
    ...
    public void validateStatusTransition(BookingStatus current, BookingStatus next) {
        ...
        if (current == BookingStatus.COMPLETED) {
            throw new ApiException(
                    ...
                    "Completed booking cannot change state"
            );
        }
